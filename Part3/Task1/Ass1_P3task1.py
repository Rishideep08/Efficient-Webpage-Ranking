...
spark = SparkSession.builder.appName("hdfs_UnOptimizedVersion").getOrCreate()
...
#reducing based on the key.
combinedlinksData_rdd = linksData_rdd.reduceByKey(lambda v1, v2: v1+v2)

#initializing the ranks
ranks_rdd= combinedlinksData_rdd.map(lambda data: (data[0],1))
...
for i in range(10):
  ...
  ranks_rdd = contributions_rdd.reduceByKey(lambda v1,v2: v1+v2)
